...
function getFilteredEvents() {
    if (!eventSearch || !eventType || !eventStatus)
        return [];
...
function updateMobileTabs() {
    ...
    buttons.forEach((button) => {
        button.classList.toggle("is-active", button.dataset.statusFilter === eventStatus.value);
    });
}
function initMobileStatusTabs() {
    const buttons = document.querySelectorAll("[data-status-filter]");
    if (!eventStatus || buttons.length === 0)
        return;
    buttons.forEach((button) => {
