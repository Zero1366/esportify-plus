...
const adminLivePanel = document.querySelector("#adminLivePanel");
const adminLastAction = document.querySelector("#adminLastAction");
const adminRequestsCount = document.querySelector("#adminRequestsCount");
const adminReportsCount = document.querySelector("#adminReportsCount");
const adminPlayersCount = document.querySelector("#adminPlayersCount");
const adminResolvedCount = document.querySelector("#adminResolvedCount");
const adminRequestsList = document.querySelector("#adminRequestsList");
let currentStep = 0;
let isLivePlaying = false;
let liveTimer = null;
let openedPlayerActionsId = null;
const ignoredIncidentPlayerIds = new Set();
const liveSteps = [
...
function syncPlayersWithLiveStep() {
...
function renderStats() {
    ...
    if (adminRequestsCount)
        adminRequestsCount.textContent = String(pendingRequests);
    if (adminReportsCount)
        adminReportsCount.textContent = String(pendingReports);
    if (adminPlayersCount)
        adminPlayersCount.textContent = String(adminPlayers.length);
    if (adminResolvedCount)
        adminResolvedCount.textContent = String(resolvedActions);
}
...
function renderPlayerCell(player) {
    ...
    const riskClass = player.risk === "high"
        ? "is-danger"
        : player.risk === "medium"
            ? "is-warning"
            : "";
...
function initMobileTabs() {
    const buttons = document.querySelectorAll("[data-mobile-view-button]");
    const panels = document.querySelectorAll("[data-mobile-view-panel]");
